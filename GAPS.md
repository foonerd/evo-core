...
Status: **closed** (see Resolution Log: Phase 2 (partial), Phase 2 tightening, Phase 2 second tightening, and Phase 2 closure entries). Every numbered Phase 2 gap resolves to IMPLEMENTED or OUT OF SCOPE.
...
- [12] Trust class OS enforcement. **Closed** - IMPLEMENTED in-scope (opt-in `[plugins.security]` UID/GID on out-of-process spawns, Unix) + OUT OF SCOPE out-of-scope (seccomp / capabilities / namespaces / LSM / Android sandbox; distribution-owned per `PLUGIN_PACKAGING.md` §5 and `BOUNDARY.md` §6.2). See gap [12] body.
- [23] Manifest resource and prerequisite declarations. **Closed** - IMPLEMENTED in-scope (`evo_min_version` and `os_family` at admission) + OUT OF SCOPE out-of-scope (`max_memory_mb`, `max_cpu_percent`, `outbound_network`, `filesystem_scopes`; distribution-owned per `PLUGIN_PACKAGING.md` §2 "Enforcement scope"). See gap [23] body.
- [20] evo-plugin-tool: **Closed (v1)** - `crates/evo-plugin-tool` implements `lint` / `sign` / `verify` / `pack` / `install` per `PLUGIN_TOOL.md`. `uninstall` / `purge` and operator-shelf work remain **Phase 5** (see gap body).

LE-1 (mock harness) is optional in this phase; typically better after SDK surfaces stabilise. If still open at Phase 2 close, it slides to Phase 7 per the Phased Execution Order's Phase 7 section.
...
- [20] evo-plugin-tool **remainder (optional hardening)**: `uninstall` / `purge`, operator-UI integration, and any stricter `install` staging policy if product needs it. Core v1 author workflow is already in `crates/evo-plugin-tool` (see [20] body).
...
- Status: RESOLVED — IMPLEMENTED (in-scope: opt-in `[plugins.security]` UID/GID on OOP spawns) + OUT OF SCOPE (out-of-scope: seccomp, Linux capabilities, network namespaces, SELinux, Android sandbox, and every other OS-level isolation primitive beyond `setuid`/`setgid`)
- Decision: split enforcement by what the steward can apply portably from a Rust process that does not claim privilege it does not need.
  - **In scope, IMPLEMENTED**: per-effective-trust-class Unix UID and GID applied to out-of-process children via `StewardConfig.plugins.security`. Opt-in, default off, Unix-only. See `AdmissionEngine::set_plugins_security`, `admit_out_of_process_from_directory` spawn path in `crates/evo/src/admission.rs`, and `docs/engineering/CONFIG.md` section 3.
  - **Out of scope, distribution-owned**: seccomp profiles, Linux capability sets, network / mount / user / PID namespaces, SELinux policy, AppArmor profiles, Android app-level sandboxing, any hypervisor or Wasm-host isolation beyond what Wasm plugins configure for themselves. The evo-core steward does not install these, does not require them, and does not orchestrate them. A distribution binds each trust class to its chosen isolation layer in the image and systemd units it ships (see `PLUGIN_PACKAGING.md` section 5 and `BOUNDARY.md` section 6.2).
- Notes: Bit-perfect / single-service-user products may leave `[plugins.security]` off; security-freak or multi-tenant packagers can turn it on. Android/automotive shapes use their own sandbox; this file documents the Linux reference path. A future in-scope widening would have to add OS primitives the steward itself owns (for example, the steward orchestrating cgroups v2 unit directives against its own children); as of v0.1.x that is OUT OF SCOPE and the distribution layer is the integration point.
...
- Promised: PLUGIN_PACKAGING section 9 describes evo-plugin-tool with lint, sign, verify, pack, install, uninstall, purge commands. `pack` / `install` archive formats: **tar.gz** (default), **tar.xz**, and **zip** (see §9 "Plugin bundle archives"). Section 7 documents **Strategy A** (installer writes final paths) vs **Strategy B** (stage at `/var/lib/evo/plugin-stage/`, then `install` does verify + promote to `search_roots`); `install` must not half-place bundles in a `search_root`.
- On disk: **`crates/evo-plugin-tool`** (binary `evo-plugin-tool`); v1 is **`lint` `sign` `verify` `pack` `install`**. `uninstall` and `purge` are **not** in this crate (Phase 5).
- Author hits: (closed for v1 author workflow) lint/sign/verify/pack/install and archive formats per `PLUGIN_TOOL.md` / `PLUGIN_PACKAGING` §7 (Strategy B) / §9.
- Status: **RESOLVED (v1)**. Remaining: **uninstall** / **purge** / operator-shelf follow-ups → Phase 5 (or a new gap) when the shelf and lifecycle are aligned; see `PLUGIN_PACKAGING` §7.
- Decision: v1 is implemented per `docs/engineering/PLUGIN_TOOL.md` (no duplicate crypto: `evo_trust` + `evo-plugin-sdk` only). Distributions still install the binary to `/opt/evo/bin/evo-plugin-tool` beside `evo` (normative in `PLUGIN_TOOL` §10).
- Notes: Encrypted signing keys remain out of v1 unless a shared `evo_trust` path already exists. Optional `--chown` delegates to the system `chown(1)` on Unix. URL downloads are capped (`--max-url-bytes`, default 256 MiB) per v1 I/O-safety.
...
- On disk: two fields (`evo_min_version`, `os_family`) are enforced at admission in `crates/evo/src/admission.rs::check_manifest_prerequisites`, called from every `admit_*` entry point right after `manifest.validate()?`. The SDK exposes the check as `evo_plugin_sdk::manifest::Manifest::check_prerequisites(evo_version, host_os)` with variants `ManifestError::EvoVersionTooLow { required, running }` and `ManifestError::OsFamilyMismatch { required, running }`. The other four fields (`outbound_network`, `filesystem_scopes`, `max_memory_mb`, `max_cpu_percent`) are parsed, preserved on the manifest, and made available to the distribution; they are not enforced from core. `PLUGIN_PACKAGING.md` section 2 ("Enforcement scope") documents the split normatively.
- Operator hits: a plugin declaring `evo_min_version = 1.0.0` is refused by a steward at 0.1.x. A plugin declaring `os_family = "linux"` is refused on a macOS / FreeBSD host. A plugin declaring `max_memory_mb = 16` is held to that by whatever `MemoryMax=` the distribution's systemd unit enforces, not by the steward.
- Status: RESOLVED — IMPLEMENTED (in-scope: `evo_min_version` and `os_family` at admission) + OUT OF SCOPE (out-of-scope: `outbound_network`, `filesystem_scopes`, `max_memory_mb`, `max_cpu_percent` — distribution-owned per the same split as [12])
- Decision: split enforcement by what core can apply portably.
  - **In scope, IMPLEMENTED**: `prerequisites.evo_min_version` (strict-less-than semver check against the steward's own `env!("CARGO_PKG_VERSION")`) and `prerequisites.os_family` (equality with `std::env::consts::OS`, or the special value `"any"`). Both checks live in `Manifest::check_prerequisites` and are called from admission via `check_manifest_prerequisites(&manifest)?` at every admit path, before shelf lookup. Covered by eight SDK unit tests in `crates/evo-plugin-sdk/src/manifest.rs` and five admission integration tests in `crates/evo/src/admission.rs` (boundary / future / mismatch / `"any"` / specific-OS / ordering).
  - **Out of scope, distribution-owned**: `prerequisites.outbound_network`, `prerequisites.filesystem_scopes`, `resources.max_memory_mb`, `resources.max_cpu_percent`. Enforcement of these requires cgroups v2, network / mount namespaces, bind mounts, or LSM policy that a userspace Rust process running as the steward user cannot apply to its own children without privilege escalation the framework does not claim. Distributions apply them via systemd unit directives (`MemoryMax=`, `CPUQuota=`, `RestrictAddressFamilies=`, `ProtectSystem=`, `ReadWritePaths=`), cgroup manager orchestration, or image policy. `PLUGIN_PACKAGING.md` section 2 ("Enforcement scope") documents the split; the manifest retains the fields so a distribution packager has contract text to hold the plugin author to.
- Notes: The scope split mirrors [12] (same pattern, different OS primitives): the evo-core steward owns the portable, Rust-enforceable half; the distribution owns the kernel-visible half. A future widening would have to add OS primitives the steward itself orchestrates (for example, the steward writing systemd drop-in files that pin `MemoryMax=` against each admitted plugin's cgroup); as of v0.1.x that is OUT OF SCOPE and the distribution's image build is the integration point.
...
Outcome: [13] and [14] RESOLVED — IMPLEMENTED; [12] PARTIAL (opt-in `plugins.security`); [20] closed v1 in a separate commit (see that Resolution Log entry); [23] remains OPEN in Phase 2.

Summary: New `crates/evo-trust` provides install digest, `manifest.sig` ed25519 verification, PEM + `*.meta.toml` trust roots, name-prefix and `max_trust_class` authorisation with `degrade_trust` policy, and `revocations.toml` digest set. `StewardConfig` `[plugins]` adds `trust_dir_opt`, `trust_dir_etc`, `revocations_path`, and `degrade_trust`. `AdmissionEngine` holds `Option<Arc<PluginTrustState>>` and `set_plugin_trust`; the binary always loads trust. Harnesses that omit `set_plugin_trust` skip crypto (existing tests). Later: `StewardConfig` `[plugins.security]`, `AdmissionEngine::set_plugins_security`, and Unix setuid/spawn for mapped trust classes (see [12] PARTIAL). Remaining in Phase 2: manifest resource enforcement ([23]). Deeper per-tier seccomp / namespace / capability enforcement remains distribution-owned per `PLUGIN_PACKAGING.md` §5 and is not required to close [12] further in core.
...
### Phase 2 second tightening - empirical closure for [20] v1 and [12] PARTIAL, bug fixes, hygiene

Outcome: IMPLEMENTED. Post-closure cleanup for Phase 2 gaps [20] v1 and [12] PARTIAL. No gap re-opens; eleven items recorded here, matching the shape of the "Phase 2 tightening" entry above for [13] / [14]. `crates/evo-plugin-tool/` moves from structurally complete (v1 surface implemented) to empirically verified (18-test integration suite driving the compiled binary, every documented exit code asserted, every normative rule from `PLUGIN_TOOL.md` sections 2, 3, and 8 checked).

Summary: verification identified eleven items worth closing before declaring Phase 2 empirically done on the [20] / [12] surface.

1. `evo-plugin-tool --degrade-trust` could not be disabled from the CLI. Clap's default `SetTrue` action on a `bool` field combined with `default_value_t = true` is one-way: the flag sets the field to true when present and defaults to true when absent. The strict-trust path (`TrustOptions { degrade_trust: false }`) was therefore unreachable from the tool even though the library layer supported it and was tested at that level by `crates/evo-trust/tests/verify.rs::declared_class_above_key_max_refuses_in_strict_mode`. Replaced with `--strict-trust` (off by default, opts in to refusal) on both Verify and Install subcommands; call sites invert via `degrade_trust: !strict_trust`. This matches the universal CLI convention of "opt in to the stricter behaviour" for bool flags and restores parity between the tool and the library layer.

2. Clap usage errors exited with status 2 by default, colliding with the documented trust / signature exit 2 (PLUGIN_TOOL.md section 8). A user running `evo-plugin-tool bogus-subcommand` saw exit 2, indistinguishable at the shell level from a real `TrustError`. `main` now uses `Cli::try_parse()` and maps clap error kinds: `DisplayHelp` / `DisplayVersion` / `DisplayHelpOnMissingArgumentOrSubcommand` -> exit 0; every other CLI-usage error -> exit 1 (usage, per section 8). The exit-code contract in `PLUGIN_TOOL.md` is now honoured uniformly across library-originated and clap-originated errors.

3. Zip pack and extract silently dropped Unix mode bits. `archive.rs`'s `pack_zip` wrote entries with `SimpleFileOptions::default()`, which carries no permissions field; `extract_zip` called `fs::File::create` and never touched the output mode. A bundle packed with `--format zip` and installed landed with `plugin.bin` at mode `0644` - unexecutable. The tar formats were unaffected because tar preserves mode natively. Fix: pack reads mode via `fs::metadata(path).permissions().mode()` and applies via `SimpleFileOptions::unix_permissions(mode & 0o777)`; extract reads the zip entry's `unix_mode()` and applies via `std::fs::Permissions::from_mode` after the file handle is closed. Both paths `#[cfg(unix)]`-guarded with no-op non-Unix fallback. The semantic equivalence claimed by `PLUGIN_PACKAGING.md` section 9 ("Unpacking any of the three formats for the same input directory yields the same file tree") is now actually true for Unix mode, matching what the tar formats already provided.

4. `install` was not atomic. The original code ran `copy_dir_all(&bundle, &dest)` directly into the final search-root destination; a mid-copy I/O failure (disk full, read error on a source file, interrupted by `SIGKILL`) left a partial tree in a `search_root`, violating `PLUGIN_TOOL.md` section 3: "Failing operations must not leave a partial or half-valid tree inside any configured `search_roots`." Replaced with a stage-and-rename pattern: copy to a hidden sibling `.<plugin.name>.installing.<pid>` first, then `fs::rename` into `dest` after the copy succeeds. Same-filesystem rename is atomic on every Primary target. If the process crashes between `remove_dir_all(&dest)` and `rename`, staging is intact and the next install run clears stale staging deterministically before retrying. The staging directory's leading dot and `.installing.` suffix ensure it is not mistaken for an admission candidate by the steward's discovery walker.

5. `install::chown_tree` wrapped `Command::status()`'s `io::Error` with `anyhow::anyhow!("chown failed: {e}")`, which formats the error as a string but loses the `io::Error` pedigree in the anyhow chain. `exit_code::code_from_error`'s `e.is::<io::Error>()` check therefore failed, and a chown launch failure (`ENOENT` on `/usr/bin/chown`, `EACCES`, etc.) emitted exit 1 (usage) instead of exit 3 (I/O) as documented. Replaced with `.with_context(|| format!("invoke chown(1) on {}", path.display()))?` which preserves the root `io::Error` through the anyhow chain for correct downcast.

6. Empirical coverage for the tool was one smoke test checking `--help` exit 0. Extended to `crates/evo-plugin-tool/tests/cli_integration.rs`: 18 tests that drive the compiled binary via `std::process::Command` (`env!("CARGO_BIN_EXE_evo-plugin-tool")`), covering every documented exit code (0 / 1 / 2 / 3), the full sign-then-verify round-trip for happy and sad paths, all three archive formats with Unix mode preservation on the installed `plugin.bin` (including the previously-failing zip path), install from a local directory, install rejection of unsigned bundles with the empty-search-root invariant asserted, the `PLUGIN_TOOL.md` section 2 archive-top-level-rename rule (via a test archive packed with a mismatched root name using the `tar` and `flate2` crates directly), the staging-directory-not-left-behind invariant on success, and the clap-error exit-code remapping for unknown subcommands and unknown flags. Deterministic ed25519 seeds (`[0xAA; 32]`, `[0xBB; 32]`) keep failures reproducible across CI runs. The whole file is `#![cfg(unix)]`-gated; Primary targets in `BUILDING.md` section 3 are Linux-glibc and Linux-musl, both Unix. The old `tests/cli_smoke.rs` is removed - its `--help` case is now one of the integration tests, alongside `--version`, unknown-subcommand, and unknown-flag tests that pin the clap remapping behaviour.

7. `crates/evo-trust/src/lib.rs` module rustdoc was stale. The previous text stated "OS-level trust class enforcement (gap [12]) is out of this crate: this library only determines the effective [`TrustClass`] the steward should apply; the process still spawns with the same user until [12] lands." With [12] PARTIAL-landed, the "until [12] lands" qualifier was no longer accurate - the process now spawns as a mapped UID/GID if `[plugins.security]` is enabled. Rewrote to describe the scope split: this crate determines the effective `TrustClass`; UID/GID application lives behind `[plugins.security]` in `crates/evo/src/admission.rs`; seccomp, capabilities, namespaces, and any deeper sandboxing remain distribution-owned.

8. `install`'s URL path error was unclear: on a non-archive URL body, the operator saw "unrecognised archive: /tmp/xxx/download" with no indication that the problem was the HTTP response, not a local file. Wrapped `archive::extract` in a `.with_context(...)` that names the URL and the accepted archive formats, so the diagnostic reads "downloaded URL <source> did not contain a recognised plugin archive (.tar.gz / .tar.xz / .zip)".

9. `evo-plugin-tool` Pack subcommand `--format` help text read "tar_gz, tar_xz, zip", but clap's `ValueEnum` derive renders variants in kebab-case, so the actual accepted values are `tar-gz, tar-xz, zip`. Corrected the help string to match what the user types.

10. `evo-plugin-tool/Cargo.toml` gained a `[dev-dependencies]` block. Binary-only crates do not expose `[dependencies]` to integration tests, so the new test suite needed its own declarations for `ed25519-dalek` (with `std`, `pkcs8`, `pem`, `alloc` features), `pkcs8` (with the `pem` feature for `LineEnding`), `tempfile`, `tar`, and `flate2`. Test fixtures use these to generate deterministic ed25519 keys, write PKCS#8 PEM private keys the tool reads via `--key`, and build archives with custom top-level names for the archive-rename test.

11. This `GAPS.md` Resolution Log's "Phase 2 (partial) - [13] and [14]" entry (the immediate predecessor of the earlier "Phase 2 tightening" entry) had a stale outcome line and a stale "Remaining:" sentence that still listed [20] as a remaining Phase 2 item, even though [20] closed v1 in an earlier commit. Outcome line corrected to acknowledge [20] closed v1 and [23] as the only remaining numbered Phase 2 gap; the "Remaining:" sentence rewritten to name [23] as the remaining work and to cross-reference `PLUGIN_PACKAGING.md` §5 for the out-of-scope split (deeper per-tier seccomp / namespace / capability enforcement is distribution-owned, not a gap against core).

Changes: `crates/evo-plugin-tool/src/main.rs` (`--strict-trust` flag, `Cli::try_parse()` exit-code remap, help-string fix), `crates/evo-plugin-tool/src/archive.rs` (zip mode preservation on pack and extract, factored `pack_zip` / `extract_zip` / `zip_file_opts` / `apply_unix_mode` helpers, module-level rustdoc added), `crates/evo-plugin-tool/src/install.rs` (stage-and-rename atomicity, `.with_context(...)` for chown io::Error, URL extract contextual error), `crates/evo-plugin-tool/Cargo.toml` (`[dev-dependencies]` block added), `crates/evo-plugin-tool/tests/cli_integration.rs` (new, 18 tests), `crates/evo-plugin-tool/tests/cli_smoke.rs` (removed), `crates/evo-trust/src/lib.rs` (module rustdoc rewritten for [12] PARTIAL scope split), `CHANGELOG.md` ([Unreleased] bullet added), `GAPS.md` (this file: "Phase 2 (partial)" entry outcome and "Remaining:" corrected; this entry appended).

Verification: the standard gate (`cargo fmt --all -- --check`, `cargo clippy --workspace --all-targets --locked -- -D warnings`, `cargo test --workspace --all-targets --locked`, `cargo build --workspace --locked`) must pass. The 18-test integration suite validates every behavioural change in items 1-5 and pins the exit-code contract from items 1, 2, and 5.

### Phase 2 closure - [23] split resolution, [12] PARTIAL reconciliation, admission call-site ordering, doc update

Outcome: [23] RESOLVED - IMPLEMENTED (in-scope) + OUT OF SCOPE (out-of-scope); [12] RESOLVED - IMPLEMENTED (in-scope) + OUT OF SCOPE (out-of-scope, reconciled from prior PARTIAL). Phase 2 is now closed: every numbered gap in the Phase 2 block of the Phased Execution Order resolves to either IMPLEMENTED or OUT OF SCOPE. The PARTIAL label is gone from the project.

Summary: the SDK and admission-side code for gap [23]'s in-scope half (`prerequisites.evo_min_version` and `prerequisites.os_family` enforcement) had already landed in an earlier uncommitted-at-the-gap-level pass: `Manifest::check_prerequisites(evo_version, host_os)` with `ManifestError::EvoVersionTooLow` / `OsFamilyMismatch` variants, and `check_manifest_prerequisites(&manifest)?` wired into all five admission entry points, with thirteen combined tests (eight in the SDK, five in `admission.rs`). The gap inventory and the framework-vs-distribution enforcement doc had not been updated to match the code. This entry records the reconciliation and closes the remaining Phase 2 work.

1. `crates/evo/src/admission.rs` call-site ordering inconsistency. Four of the five admission entry points (`admit_singleton_respondent`, `admit_singleton_warden`, `admit_out_of_process_respondent`, `admit_out_of_process_warden`) called `check_manifest_prerequisites(&manifest)?` AFTER shelf lookup, shape check, and duplicate-shelf check. The fifth (`admit_out_of_process_from_directory`) correctly called it immediately after the manifest parse, before transport checks. A manifest with both a bad shelf and a future `evo_min_version` would therefore produce different error precedence on different admission paths: from-directory surfaced the version error, the other four surfaced the shelf error. Moved the call in the four affected paths to immediately after `manifest.validate()?`, before shelf lookup. Rationale: "can this steward run this plugin at all" (wrong OS, framework version too old) is a more fundamental gate than "is this shelf in the catalogue today"; the more fundamental gate should fire first so operators see the actionable error. All existing tests continue to pass because the fixtures admit against valid shelves; the reorder only changes precedence when multiple violations are present.

2. `docs/engineering/PLUGIN_PACKAGING.md` section 2 "Enforcement scope". Added a new subsection after the manifest schema listing. The subsection contains two tables: fields enforced by the evo-core steward at admission (with the exact enforcement mechanism per field), and fields that are distribution-owned (with the typical enforcement point per field: systemd directive, cgroup, namespace, LSM, etc.). Closes the documentation side of [23]'s scope split normatively, so future plugin authors and distribution packagers have a single source of truth for which manifest fields core enforces. The subsection cross-references `BOUNDARY.md` section 6.2 for the broader framework-vs-distribution line.

3. `GAPS.md` gap [23] entry. Status: OPEN -> RESOLVED. `On disk` rewritten to describe the implemented code path (naming the SDK method, the admission helper, and the error variants). `Operator hits` rewritten to describe what admission rejects today (future `evo_min_version`, mismatched `os_family`) versus what remains distribution-enforced (resource caps, filesystem, network). `Decision` split explicitly into **In scope, IMPLEMENTED** (evo_min_version, os_family; test counts named) and **Out of scope, distribution-owned** (max_memory_mb, max_cpu_percent, outbound_network, filesystem_scopes; typical OS enforcement mechanisms named). `Notes` draws the parallel with [12]'s same-shape split (portable Rust half in core, kernel-visible half in distribution) and names the hypothetical future widening (the steward itself orchestrating cgroups / systemd drop-ins) as explicitly OUT OF SCOPE for v0.1.x so the gap cannot re-open on a reading of "but you could do more".

4. `GAPS.md` gap [12] entry. Status: PARTIAL -> RESOLVED. The `PARTIAL` label violated the project's own Standard rule ("Every gap below resolves to exactly one of two outcomes. There is no third option. [...] 'Deferred', 'pending', 'future pass', 'not yet', 'v0 only' are not acceptable labels."). Reconciled to `RESOLVED - IMPLEMENTED (in-scope: opt-in [plugins.security] UID/GID on OOP spawns) + OUT OF SCOPE (out-of-scope: seccomp, Linux capabilities, network namespaces, SELinux, Android sandbox, and every other OS-level isolation primitive beyond setuid/setgid)`. The `Decision` field now enumerates both halves with the same level of detail as [23]; the code that already existed (`AdmissionEngine::set_plugins_security`, `admit_out_of_process_from_directory` spawn path, `PluginsSecurityConfig::uid_gid_for_class`) is explicitly named as the in-scope IMPLEMENTED half. The out-of-scope half names every isolation primitive the framework declines to own. `Notes` draws the same parallel with [23] and names the hypothetical future widening (the steward orchestrating its own cgroups) as explicitly OUT OF SCOPE.

5. `GAPS.md` Phased Execution Order, Phase 2 block. Status changed from `partial` to `closed`. Per-gap bullets updated: [12] and [23] now read `**Closed** - IMPLEMENTED in-scope + OUT OF SCOPE out-of-scope` with the scope split named; both cross-reference `PLUGIN_PACKAGING.md` (section 5 for [12], section 2 for [23]) and gap-body detail. LE-1 sentence extended to explicitly name the Phase 7 slide path so the eventual Phase 7 entry is not surprising.

Changes: `crates/evo/src/admission.rs` (four admit_* paths: `check_manifest_prerequisites(&manifest)?` call moved to immediately after `manifest.validate()?`; `admit_out_of_process_from_directory` unchanged, already correct), `docs/engineering/PLUGIN_PACKAGING.md` section 2 (new "Enforcement scope" subsection with two normative tables and a closing paragraph on core's position), `GAPS.md` (this file: gap [12] entry Status/Decision/Notes rewritten for IMPLEMENTED+OOS split; gap [23] entry Status/On disk/Operator hits/Decision/Notes rewritten for IMPLEMENTED+OOS split; Phase 2 execution-order block status `partial` -> `closed`, per-gap bullets updated; this Resolution Log entry appended), `CHANGELOG.md` ([Unreleased] bullet added).

Verification: the standard gate (`cargo fmt --all -- --check`, `cargo clippy --workspace --all-targets --locked -- -D warnings`, `cargo test --workspace --all-targets --locked`, `cargo build --workspace --locked`) must pass. The existing thirteen [23]-related tests (eight SDK unit tests in `crates/evo-plugin-sdk/src/manifest.rs`, five admission tests in `crates/evo/src/admission.rs`) continue to pass unchanged after the call-site reorder - they exercise the behaviour end-to-end at every admission entry point indirectly, and explicitly at `admit_singleton_respondent`.

### Phase 2 status

Phase 2 is now **closed**. Every numbered gap in the Phase 2 block ([12], [13], [14], [20], [23]) resolves to IMPLEMENTED or OUT OF SCOPE. LE-1 remains OPEN as an optional phase-2 item; per the Phased Execution Order's Phase 7 section, it slides to Phase 7 if it has not closed by the end of Phase 2, which is now. v0.1.9 cannot tag until LE-1 closes (IMPLEMENTED or OUT OF SCOPE) along with every remaining numbered gap from Phases 3-6.

...
**Status: implemented.** `RelationGraph::suppress`, `::unsuppress`, and `::is_suppressed` are the storage primitives; `RegistryRelationAdmin::suppress` and `::unsuppress` are the wiring-layer surface. Suppression is implemented by index cleanliness: while suppressed, the edge is absent from the forward and inverse indices, so neighbour queries, walks, and cardinality counts silently skip it without per-call filtering. Re-suppressing an already-suppressed relation is a silent no-op preserving the original suppression record; unsuppressing a non-suppressed or unknown relation is also a silent no-op. `Happening::RelationSuppressed` and `::RelationUnsuppressed` fire on the bus on the visibility-changing transitions only. Audit entries are recorded in the steward's `AdminLedger` for each visibility change.

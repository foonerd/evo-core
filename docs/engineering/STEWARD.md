...
Plugins request user-facing prompts through `LoadContext::user_interaction_requester`. The steward parks the request on the `PromptLedger` and routes it to the consumer connection holding the negotiated `user_interaction_responder` capability (single-claimer; first-claimer-wins). On the Unix-socket path the claim releases automatically via an RAII guard when the connection closes. On the HTTPS-arriving path — where each REST/WS wire op fabricates a fresh `ConnectionState` and no single connection lifetime exists — the responder identity is derived from a stable salted hash of `Principal.token_id`, so every op carrying the same bearer maps to the same claim (idempotent same-holder renegotiate; different-bearer refused with `responder_already_assigned`) and release is bounded by bearer TTL rather than steward lifetime. The operator UI can short-circuit the TTL wait by calling `release_user_interaction_responder` on explicit log-out or tab-close. The responder consumer seeds the currently-Open prompt set with `list_user_interactions` and tracks live updates by subscribing to the framework's happenings bus (`op = "subscribe_happenings"`) and filtering for `UiShelfChanged` events on the `prompts.active` shelf: each prompt auto-stocks a widget on that shelf on issue (`Happening::UiShelfChanged { shelf: "prompts.active", ... }`) and unstocks on resolution, so the renderer sees every transition it needs to observe without a dedicated prompt-subject subscription. The consumer answers via `answer_user_interaction` or `cancel_user_interaction`. The plugin's awaiting future resolves when the consumer responds (or with `PromptOutcome::Cancelled` on cancel / disconnect / timeout).

The single-responder property is enforced at the op layer, not just at negotiate: `list_user_interactions` / `answer_user_interaction` / `cancel_user_interaction` refuse when the caller's connection is not the current slot holder (structured subclasses `responder_slot_unclaimed` for the never-negotiated case, `responder_slot_held_by_other` for the losing-race case). `release_user_interaction_responder` uses a lighter gate (capability only, not slot ownership) because its ledger call is idempotent same-holder-only clear.

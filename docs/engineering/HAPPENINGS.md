...
- `SubjectMerged` and `SubjectSplit` fire BEFORE the per-edge cascade variants the rewrite produces. The order within a single parent is fixed and pinned by tests. For `SubjectMerged`: parent envelope → all `RelationRewritten` events (one per affected edge, source_a's edges then source_b's) → any `RelationCardinalityViolatedPostRewrite` (one per `(subject_id, predicate, side)` whose count exceeds the declared cardinality after rewrite, in stable subject-sorted order) → any `RelationClaimSuppressionCollapsed` (one per demoted claim where a rewrite collided onto a suppressed surviving edge) → all `ClaimReassigned` events (one per relation claim per affected edge, then one per addressing transferred at the registry level). For `SubjectSplit`: parent envelope → all `RelationRewritten` events → any `RelationSplitAmbiguous` events (one per edge the strategy could not place) → any `RelationCardinalityViolatedPostRewrite` → all `ClaimReassigned`. Suppression-collapse does not arise in split (split distributes outward; no collision-with-suppressed scenario).
...
4. If the request carried a `since` cursor, queries `happenings_log` for every event with `seq > since` and streams those replay frames in ascending seq order.
...
Wire-level details and the cursor surface contract are in `STEWARD.md` section 6.2.
...
5. Every emitted happening carries a strictly monotonic `seq: u64`. The seq is durable across restart for events written via `emit_durable`; cursor-aware consumers reconnect with their last-observed seq as `since` and resume cleanly.

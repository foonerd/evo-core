...
| Option | Trade-off |
|--------|-----------|
| Steward-owned (sled table, reconciled against running wardens at boot) | Steward carries the data; plugins stay stateless on this axis. Reconciliation is non-trivial when warden state and steward state diverge. |
| Plugin-owned (wardens persist and re-announce on load) | Plugins carry the data; steward stays stateless. Wardens without persistent storage (e.g. ephemeral ones) cannot participate. |
| Hybrid (steward persists enough to re-contact wardens; wardens persist full state) | Best of both; complexity of two persistence boundaries. |

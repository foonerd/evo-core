...
`plugins.d/<plugin_name>.toml` is the operator-controlled config the steward merges into the plugin's `LoadContext.config` at admission time. The file name MUST match the plugin's canonical `plugin.name` exactly, with `.toml` appended (e.g. `org.example.myplugin` → `org.example.myplugin.toml`). The steward looks the file up by exact filename match; a mismatched stem reaches the plugin as an empty config table.

The contract:

- **Missing file** is not an error. The plugin sees an empty `toml::Table` in `LoadContext.config` and is expected to operate on its own defaults. An operator who has not authored a config for this plugin pays nothing.
- **Present-but-malformed file** (TOML parse error, including the typical operator typos: unclosed string, broken section header, illegal `=` placement) **aborts admission of that plugin**. The steward refuses to load a plugin with a broken operator-supplied config rather than silently substituting an empty table — the silent fall-through would mask the operator's intent and produce confusing runtime behaviour. The error message names the plugin and the offending file path so the operator can find the typo without grepping.
- **File contents are not validated by the steward.** The steward parses the TOML into a generic `toml::Table` and hands it to the plugin verbatim. The plugin is responsible for validating its own config schema and refusing to load (returning a `PluginError`) if the operator's config does not satisfy it.

The directory itself is operator-owned. Distributions ship their own per-plugin defaults inside the bundle (typically as a TOML file the plugin reads on first boot if the operator has not supplied an override). The `plugins.d/` overlay is the operator surface, not the distribution surface.

The drop-in directory location is configurable via `plugins.config_dir` in `evo.toml`; default `/etc/evo/plugins.d/`.
...
Plugin signing (above) is the per-bundle authorisation surface — every plugin artefact carries a `manifest.sig` verified against the trust root. Releases of the **steward binary itself**, **reference-device binaries**, and **vendor distribution binaries** layer additional trust roots above the plugin trust root. Each release plane (per `RELEASE_PLANE.md`) signs against its own root; the verification chain on a device is `release root → plugin trust root → plugin signature`.

Four release-tier roots are recognised. Each lives in its publishing repository under `keys/<role>-public.pem` with a sibling `*.meta.toml` declaring authorisation; the corresponding private half stays in the publisher's CI secret store.

| Role | Source repository | `name_prefixes` it authorises | Trust class | Used to sign |
| ---- | ----------------- | ----------------------------- | ----------- | ------------ |
| `framework_release_root` | `foonerd/evo-core` | `org.evoframework.core.*` | `platform` | The `evo` steward binary, `evo-plugin-tool`, every artefact published through `foonerd/evo-core-artefacts`. |
| `reference_device_release_root` | The reference-device repository (`foonerd/evo-device-audio` for the audio domain) | `org.evoframework.<domain>.*` | `platform` | Reference-device images, the device's plugin bundles published through the reference-device's artefacts repo. |
| `vendor_release_root` | Vendor's own distribution repository | Vendor's own namespace (e.g. `com.<vendor>.*`) | `privileged` or lower (vendor's call) | Vendor distribution images, vendor-published plugin bundles. |
| `commons_release_root` | The reference-device repository | `org.evoframework.*` (broader than the device-specific namespace) | `platform` | Brand-neutral plugin bundles the reference device hosts on behalf of the framework's plugin commons. |

Layering rules:

- **Distinct from plugin trust roots.** A plugin's `manifest.sig` verifies against its plugin trust key (`vendor` or lower role). A release artefact's `build-info.sig` verifies against a release-tier root above. The two layers do not share keys; an operator examining a fetched artefact sees both signatures and verifies each against its own root.
- **Distinct keys per role.** The framework, reference-device, vendor, and commons release roots are four separate keypairs. Compromise of one does not bleed into the others; revocation of one does not invalidate the others.
- **Public halves pinned in source.** Each role's public half lives in the source tree of its publishing repository. An operator with the source clone has the verification material in hand; verification needs no network call to a key server.
- **Authorisation by `name_prefixes`.** Per the existing key-meta convention, each release root carries `name_prefixes` matching its publisher namespace. An attacker substituting a release artefact must produce a manifest whose `publisher` matches one of the legitimate root's prefixes AND a signature made by that root's private half — neither of which is exposed.
- **Promotion is a pointer move.** Channel pointers (`channels/<bundle-kind>/<channel>`) move between manifests under the same release root; the bytes do not change between channels and re-signing on promotion does not happen. See `RELEASE_PLANE.md` §3.1.

When a vendor publishes a distribution layered on the reference device, the device boot path verifies the vendor binary against `vendor_release_root`, the reference-device binary against `reference_device_release_root`, the framework binary against `framework_release_root`, and each admitted plugin against its plugin trust key. Four signatures, four roots, four namespaces — the verification graph is finite and an operator can audit it without external lookup.

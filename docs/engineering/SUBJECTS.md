...
Retraction removes the addressing from the subject. If the subject has no remaining addressings after retraction, it is garbage-collected (with a `SubjectForgotten` happening).
...
The registry lives under `/var/lib/evo/subjects/`. The exact file layout, backing store, and on-disk format are engineering-implementation concerns, not specified here. Directionally: an embedded ACID store (SQLite, LMDB, sled) rather than flat files, because the registry's access patterns are dominated by point lookups with strict consistency requirements.
...
| `SubjectMerged` | Two or more subjects collapse into one. Carries source IDs and new ID. |
| `SubjectSplit` | One subject becomes two or more. Carries source ID and new IDs. |
| `SubjectForgotten` | A subject is deleted. Carries ID and reason. |

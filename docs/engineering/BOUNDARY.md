...
| Vendor distribution | `evo-device-<vendor>` | Many repositories, one per product line. The catalogue choices, vendor branding, product-specific plugins (closed-source streaming-service integrations, partner certifications), commercial packaging. Imports a reference generic device of its domain and adds the vendor layer. | Vendor's own (`com.<vendor>.*`) |

The first reference generic device is `evo-device-audio`. The framework imposes no upper bound on how many domains have reference generic devices or how many vendors layer on each.

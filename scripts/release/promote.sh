...
# (e.g. publish.yml is deliberately deleted from eng — public is
# the sole owner); a wipe that copied staging over the public
# workflow directory would delete public workflows or route
# public CI to the wrong runner pool. This is a HARD invariant
# of the tool: never remove the preservation predicate without
# operator authorisation.
#
# DCO contract: the release commit built in §6 is created with
# `git commit --signoff` so the resulting commit carries a
# `Signed-off-by:` trailer. Every commit on public evo-core
# requires DCO signoff per project policy.
#
# Usage:
#
...
#
#   --public-repo PATH
#     Filesystem path to a clone of the public evo-core repo.
...
(
    ...
    # public workflow set is the canonical shipping surface and
    # differs from the eng-side (e.g. publish.yml is deliberately
    # deleted from eng); a wipe-and-copy that let staging land
    # workflows over the public tree would delete public workflows
    # or route public CI to the wrong runner pool. This is a HARD
    # invariant of the tool: never remove the preservation
    # predicate without operator authorisation.
    ...
    git add -A
    git commit --signoff -m "release ${TAG}"
    git tag "${TAG}"
)

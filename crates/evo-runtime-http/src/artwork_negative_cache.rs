...
/// Default TTL for a negative memo entry — the day-scale layer
/// per the artwork-online provider bar. Long enough that a
/// repeat browse-load of a no-cover album a few hours later
/// does not re-hammer the providers; short enough that an
/// operator who added local artwork after the memo landed
/// sees it surface without a manual refresh.
...
pub struct NegativeEntry {
    /// Plugin status token — always `"not_found"` in production
    /// (the module refuses to store other statuses), preserved
    /// here so a future call could distinguish other structured
    /// negatives if the contract ever widens.
    pub status: String,

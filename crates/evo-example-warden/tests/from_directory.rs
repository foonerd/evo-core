...
//! dispatch branch added in pass 4e-c. This is the only place where
//! that branch is exercised end-to-end with a real plugin binary.
//!
...
/// Build the catalogue with the `example.custody` shelf.
...
async fn admit_from_directory_warden_full_lifecycle() {
    ...
    // 4e-c dispatch branch: manifest.kind.interaction = "warden"
    // routes to admit_out_of_process_warden.
    engine

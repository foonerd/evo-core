...
async fn handle_connection(
    ...
    loop {
        ...
        if let ClientRequest::SubscribeHappenings { since, filter } = req {
        ...
        match rx.recv().await {
            Ok(env) => {
                ...
                if !filter.accepts(&env.happening) {
                    ...
                    // happening; the bus subscription continues, the
                    // consumer simply does not see this seq. The
                    // dedupe boundary is unaffected — live filter
                    // misses do not need to be replayed because the
                    // consumer never asked for them.
                    continue;
                }
                let frame = ClientResponse::Happening {
                    seq: env.seq,

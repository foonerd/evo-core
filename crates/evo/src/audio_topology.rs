...
use crate::topology_scoring::{ScoreBreakdown, VolumeMode};
use evo_plugin_sdk::audio::AudioFormat;
use evo_plugin_sdk::contract::audio_routing::{
    EndpointKind, ReadEndpoint, WriteEndpoint,
};
...
impl ActiveChainStage {
    ...
    pub fn role(&self) -> PluginAudioRole {
        match self {
...
pub struct ActiveAudioTopology {
    ...
    /// (validated by the framework via
    /// [`crate::topology_scoring::validate_bit_perfect`]
    /// before the snapshot is published).
    pub bit_perfect: bool,
    /// Score breakdown from
    /// [`crate::topology_scoring::score_topology`] — the
    /// auditable form so the operator UI can render the
    /// reason for each rule's score / penalty.
...
impl ActiveAudioTopology {
    /// Validate the topology's basic shape. Called by
    /// [`AudioTopologyStore::publish`] before any persistence
    /// / propagation.
    pub fn validate(&self) -> Result<(), AudioTopologyError> {
        if self.target_key.trim().is_empty() {
        ...
        if !matches!(self.chain.first(), Some(ActiveChainStage::Source { .. }))
        {
            return Err(AudioTopologyError::Validation(
                "chain must begin with a Source stage".into(),
            ));
        }
        if !matches!(self.chain.last(), Some(ActiveChainStage::Delivery { .. }))
        {
            return Err(AudioTopologyError::Validation(
        ...
        for (idx, stage) in self.chain.iter().enumerate() {
            if idx == 0 || idx == self.chain.len() - 1 {
                continue;
            }
        ...
        if let Some(p) = self.volume_position {
        ...
        // control.
        if matches!(self.volume_mode, VolumeMode::None)
            && self.volume_position.is_some()
        {
            return Err(AudioTopologyError::Validation(
...
impl AudioTopologyStore {
    ...
    pub async fn publish(
        ...
        for stage in &topology.chain {
            ...
            self.routing.publish_topology(stage.plugin(), resolved);
        }
    ...
    pub async fn clear(
        ...
        if let Some(existing) = self.get(target_key).await? {
            for stage in &existing.chain {
                self.routing.clear_topology(stage.plugin());
            }
        }
        self.persistence

...
use crate::topology_scoring::{OperatorPolicy, VolumeMode};
...
/// Typed record returned by [`AudioPolicyStore`] policy reads.
/// Carries the canonical target key + the parsed policy +
...
/// Typed record returned by [`AudioPolicyStore`] volume-mode
/// reads.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
